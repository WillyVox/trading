...
async function main() {
  ...
  const files = args.file
    ? allFiles.filter((f) => f.fileName === args.file)
    : allFiles;

  if (args.file && files.length === 0) {
    console.error(
      `File "${args.file}" not found in ${PUBLISH_DIR} (or it has an unsupported extension).`
    );
    process.exitCode = 1;
    return;
  }
  ...
  for (const file of files) {
    try {
      ...
      if (!args.noAutoImage) {
        const imageWarning = await resolveFallbackFeaturedImage(
          parsed.payload,
          {
            dryRun: args.dryRun,
      ...
      if (args.verbose) {
        console.log(
          `  [${file.fileName}] metadata: ${JSON.stringify(
            {
              ...parsed.payload,
              content: `<${parsed.payload.content.length} chars of HTML>`,
            },
            null,
            2
          )}`
  ...
  for (const entry of pending) {
    ...
    if (!args.dryRun) {
      try {
        ...
        relationshipWarnings.push(
          `article was imported but the file could not be moved: ${(err as Error).message}`
        );
      }
    }
    ...
    results.push({
      ...
      warnings: [
        ...
        ...(relationshipError
          ? [`relationship resolution failed: ${relationshipError}`]
          : []),
      ],
      dryRun: args.dryRun,
      movedTo,
  ...
  results.sort(
    (a, b) =>
      (orderIndex.get(a.fileName) ?? 0) - (orderIndex.get(b.fileName) ?? 0)
  );

  results.forEach((result, i) =>
    printFileResult(i + 1, results.length, result)
  );
