...
const OPEN_TAG_RE = new RegExp(`<${VIDEO_EMBED_TAG_NAME}\\s+([^>]*)>\\s*</${VIDEO_EMBED_TAG_NAME}>`, "gi");
...
export function editorHtmlToMarkers(html: string): string {
  return html.replace(OPEN_TAG_RE, (full, attrsRaw: string) => {
    ...
    return caption ? `{{video:${attrs.provider}:${attrs["video-id"]}:${caption}}}` : `{{video:${attrs.provider}:${attrs["video-id"]}}}`;
  });
}
