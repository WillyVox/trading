...
function ExploreMarkets({ pathname }: { pathname: string | null }) {
  ...
  return (
    <section
      aria-labelledby="research-explore-markets"
      className="border-border bg-panel rounded-2xl border p-3 shadow-sm sm:p-4"
    >
      <h2
        id="research-explore-markets"
    ...
    >
      ...
      <div className="grid grid-cols-1 items-start gap-4 lg:grid-cols-[1.15fr_0.85fr] lg:gap-5">
        <section className="border-border bg-panel rounded-2xl border p-3 shadow-sm sm:p-4">
          ...
          <div className="space-y-4 lg:grid lg:grid-cols-2 lg:space-y-0 lg:gap-x-4 lg:gap-y-5 xl:grid-cols-3">
          ...
          {guides?.footerLink && (
            ...
            >
              <span>{guides.footerLink.label.replace(/\s*→\s*$/, "")}</span>
        ...
        <section className="border-border bg-panel rounded-2xl border p-3 shadow-sm sm:p-4 lg:col-span-2">
          ...
          {tools?.footerLink && (
            <Link
              ...
              className="border-border text-navy hover:bg-panel-secondary mt-3 flex items-center justify-between border-t px-2.5 pt-3 text-sm font-bold transition-colors"
            >
              <span>{tools.footerLink.label.replace(/\s*→\s*$/, "")}</span>
              <span aria-hidden="true">→</span>
            </Link>
          )}
        </section>
  ...
  return (
    <div className={mobile ? "lg:hidden" : "hidden lg:block"}>
      <button
        ...
        aria-label={
          mobile ? "Open site navigation" : "Open quick access navigation"
        }
        ...
        <div className="border-border bg-panel shrink-0 border-b">
          <div className="mx-auto flex h-[72px] max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
            ...
            <button
              ref={closeRef}
              type="button"
              aria-label="Close quick access navigation"
...
export function MobileNav() {
  return <ResearchNavigator trigger="mobile" />;
}
export function DesktopQuickAccess() {
  return <ResearchNavigator trigger="desktop" />;
}
