...
export function StepsFlowIllustration({ title }: { title: string }) {
  ...
  return (
    <svg {...BASE_SVG_PROPS} role="img" aria-label={title}>
      <title>{title}</title>
      <line x1="80" y1="180" x2="720" y2="180" stroke={CREAM} strokeOpacity={0.25} strokeWidth={2} />
      {steps.map((n, i) => {
        const x = 80 + i * 160;
        return (
          <g key={n}>
            <circle cx={x} cy={180} r={34} fill={i === steps.length - 1 ? GOLD : "none"} stroke={GOLD} strokeWidth={3} />
            <text
              x={x}
              y={190}
              textAnchor="middle"
...
export function FeesBarIllustration({ title }: { title: string }) {
  ...
  return (
    <svg {...BASE_SVG_PROPS} role="img" aria-label={title}>
      <title>{title}</title>
      <line x1="80" y1="280" x2="720" y2="280" stroke={CREAM} strokeOpacity={0.3} strokeWidth={2} />
      {bars.map((b, i) => {
        const x = 150 + i * 140;
        return (
          <g key={b.label}>
            <rect x={x - 30} y={280 - b.h} width={60} height={b.h} rx={6} fill={b.color} />
            <text x={x} y={305} textAnchor="middle" fontFamily="'DejaVu Sans', Arial, sans-serif" fontSize={16} fill={CREAM}>
              {b.label}
            </text>
          </g>
        );
...
export function SecurityShieldIllustration({ title }: { title: string }) {
  return (
    <svg {...BASE_SVG_PROPS} role="img" aria-label={title}>
      ...
      <path d="M330 200 L365 235 L470 130" fill="none" stroke={CREAM} strokeWidth={5} strokeLinecap="round" strokeLinejoin="round" opacity={0.5} />
    </svg>
  );
}

/** Two-column side-by-side comparison bars — used for "share trading vs crypto trading". */
export function CompareColumnsIllustration({ title, labelLeft, labelRight }: { title: string; labelLeft: string; labelRight: string }) {
  return (
    <svg {...BASE_SVG_PROPS} role="img" aria-label={title}>
      <title>{title}</title>
      <line x1="400" y1="60" x2="400" y2="280" stroke={CREAM} strokeOpacity={0.25} strokeWidth={2} strokeDasharray="6,8" />
      ...
      <text x="270" y="315" textAnchor="middle" fontFamily="'DejaVu Sans', Arial, sans-serif" fontWeight={700} fontSize={20} fill={CREAM}>
        {labelLeft}
      </text>
      <text x="520" y="315" textAnchor="middle" fontFamily="'DejaVu Sans', Arial, sans-serif" fontWeight={700} fontSize={20} fill={CREAM}>
        {labelRight}
      </text>
    </svg>
  );
...
export function GrowthTrendIllustration({ title }: { title: string }) {
  return (
    <svg {...BASE_SVG_PROPS} role="img" aria-label={title}>
      <title>{title}</title>
      <line x1="80" y1="280" x2="720" y2="280" stroke={CREAM} strokeOpacity={0.25} strokeWidth={2} />
      <line x1="80" y1="60" x2="80" y2="280" stroke={CREAM} strokeOpacity={0.25} strokeWidth={2} />
      <polyline
...
export function ResearchIllustration({ title }: { title: string }) {
  return (
    <svg {...BASE_SVG_PROPS} role="img" aria-label={title}>
      <title>{title}</title>
      <rect x="280" y="60" width="180" height="240" rx="8" fill="none" stroke={CREAM} strokeOpacity={0.6} strokeWidth={3} />
      {[100, 140, 180, 220].map((y) => (
        <line key={y} x1="305" y1={y} x2="435" y2={y} stroke={CREAM} strokeOpacity={0.4} strokeWidth={3} />
      ))}
      <circle cx="500" cy="230" r="55" fill="none" stroke={GOLD} strokeWidth={7} />
      <line x1="540" y1="270" x2="590" y2="320" stroke={GOLD} strokeWidth={9} strokeLinecap="round" />
    </svg>
  );
}
