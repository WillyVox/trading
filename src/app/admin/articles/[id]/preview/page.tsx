    ...
    <div className="mx-auto max-w-3xl">
      <Notice>
        <div className="flex flex-wrap items-center justify-between gap-2">
          <span>
            Admin preview — <strong>{article.status}</strong>. Never visible to
            the public at this URL; noindex, not cached.
          </span>
          <Link
      ...
      <article className="mt-8">
        ...
        {article.keyTakeaways.length > 0 && (
          <div className="border-border bg-panel-secondary mt-6 rounded-xl border p-4">
            <h2 className="text-navy text-sm font-bold">Key takeaways</h2>
            <ul className="text-navy mt-2 list-disc space-y-1 pl-5 text-sm">
        ...
        {headings.length > 1 && (
          <nav className="border-border mt-6 rounded-xl border p-4 text-sm">
            ...
            <ol className="mt-2 list-decimal space-y-1 pl-5 text-sm">
              {article.sources.map((s, i) => (
                <li key={i}>
                  ...
                  {s.sourceType && (
                    <span className="text-muted"> · {s.sourceType}</span>
                  )}
                </li>
              ))}
            </ol>
