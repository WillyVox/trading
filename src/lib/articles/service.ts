 ...
 */
export const getRegionalFamily = cache(
  async (articleId: string, canonicalArticleId: string | null) => {
    ...
    if (!root) return [];
    return [
      { id: root.id, slug: root.slug, region: root.region },
      ...root.regionalVariants,
    ];
  }
);
 ...
 * Block 1.
 */
export const getPublishedArticleBySlugAndType = cache(
  async (slug: string, articleType: ArticleType) => {
