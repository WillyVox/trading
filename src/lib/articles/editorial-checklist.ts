...
export function buildEditorialChecklist(
  ...
  items.push(
    input.excerpt && input.excerpt.trim().length > 0
      ...
      : {
          level: "warning",
          message: "Excerpt missing — used as the listing/summary description",
        }
  );
  ...
  if (!input.seoTitle) {
    items.push({
      level: "warning",
      message: "SEO title missing — falls back to the article title",
    });
  } else if (input.seoTitle.length > 60) {
    items.push({
      level: "warning",
      message: `SEO title is ${input.seoTitle.length} chars — recommended under 60`,
    });
  } else {
  ...
  if (!input.seoDescription) {
    ...
    items.push({
      level: "warning",
      message: `Meta description is ${input.seoDescription.length} chars — recommended under 160`,
    });
  } else {
  ...
  if (input.sourceCount === 0) {
    items.push({
      level: "warning",
      message:
        "Article contains no sources — important for financial/crypto content",
    });
  } else {
    items.push({
      level: "good",
      message: `${input.sourceCount} source${input.sourceCount === 1 ? "" : "s"}`,
    });
  }

  items.push(
    input.author
      ? { level: "good", message: "Author present" }
      : { level: "warning", message: "No author set" }
  );
  items.push(
    input.reviewer
      ? { level: "good", message: "Reviewer present" }
      : { level: "warning", message: "No reviewer set" }
  );

  if (!input.lastReviewedAt) {
    items.push({
      level: "warning",
      message: "Article has never been marked as fact-checked/reviewed",
    });
  } else {
    ...
    if (daysSince > STALE_REVIEW_DAYS) {
      items.push({
        level: "warning",
        message: `Last reviewed ${daysSince} days ago — consider a fact-check pass`,
      });
    } else {
      items.push({
        level: "good",
        message: `Last reviewed ${daysSince} day${daysSince === 1 ? "" : "s"} ago`,
      });
    }
  }
  ...
  items.push(
    takeawayCount > 0
      ? {
          level: "good",
          message: `${takeawayCount} key takeaway${takeawayCount === 1 ? "" : "s"}`,
        }
  ...
  items.push(
    providerRelationshipCount > 0
      ? {
          level: "good",
          message: `${providerRelationshipCount} related provider${providerRelationshipCount === 1 ? "" : "s"} configured`,
        }
  ...
  items.push(
    input.cryptoAssetCount > 0
      ? {
          level: "good",
          message: `${input.cryptoAssetCount} related crypto asset${input.cryptoAssetCount === 1 ? "" : "s"}`,
        }
  ...
  if (hasFeaturedOrComparedProvider && !input.affiliateDisclosureRequired) {
    items.push({
      level: "warning",
      message:
        "Article features/compares a provider but affiliate disclosure is disabled",
    });
  }
  ...
  items.push({
    level: "info",
    message: `${words} words \u2022 ~${estimateReadingMinutes(input.content)} min read`,
  });

  items.push({
    level: "info",
    message: input.noIndex
      ? "noindex is ON — excluded from search engines"
      : "noindex is off — indexable once published",
  });
