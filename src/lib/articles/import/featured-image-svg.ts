...
export function pickConcept(
  ...
  const bySearchIntent = payload.searchIntent ? SEARCH_INTENT_CONCEPTS[payload.searchIntent] : undefined;
...
function assetMonogram(payload: Pick<ArticleImportPayload, "cryptoAssetSlugs">): string | null {
...
function ledgerLines(): string {
  return [200, 280, 360, 440]
    .map((y) => `<line x1="80" y1="${y}" x2="1120" y2="${y}" stroke="#ffffff" stroke-opacity="0.06" stroke-width="1" />`)
    .join("\n");
}
...
export function buildFeaturedImageSvg(
  payload: Pick<ArticleImportPayload, "searchIntent" | "articleType" | "cryptoAssetSlugs">,
