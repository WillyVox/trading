...
test("buildFeaturedImageSvg adds a coin badge only for a recognized crypto asset", () => {
  const withBitcoin = buildFeaturedImageSvg({ searchIntent: "HOW_TO", cryptoAssetSlugs: ["bitcoin"] });
  assert.match(withBitcoin, />B<\/text>/);

  const withUnknownAsset = buildFeaturedImageSvg({ searchIntent: "HOW_TO", cryptoAssetSlugs: ["some-new-token"] });
