...
export async function getCryptoFeeOfferings(): Promise<CryptoFeeOffering[]> {
  ...
  for (const fee of fees) {
    const status: CryptoFeeRule["verificationStatus"] =
      fee.verificationStatus === "VERIFIED"
        ? "VERIFIED"
        : fee.verificationStatus === "STALE"
          ? "STALE"
          : "UNVERIFIED";
    const rule: CryptoFeeRule = {
      ...
      category: fee.feeCategory as CryptoFeeRule["category"],
      label: fee.label,
      calculationBasis:
        fee.calculationBasis as CryptoFeeRule["calculationBasis"],
    ...
    grouped.set(rule.offeringSlug, current);
  }
