> **Architecture update (2026-09-22 / Phase 8.1):** Comparison is a first-class product area under `/compare`. Crypto comparisons live under `/compare/crypto-exchanges` and share-trading comparisons under `/compare/trading-platforms`. Provider-pair/multi-provider pages use canonical `a-vs-b` slugs under those category hubs. Historical milestones below may describe superseded routes.
